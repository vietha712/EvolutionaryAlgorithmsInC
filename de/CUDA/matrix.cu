                                            ...
                                            Matrix1DT* outputMatrix)
{
    ...
    if (0 == outputMatrix->isInit)
    {
        ...
        outputMatrix->rows = firstMatrix->rows; outputMatrix->cols = secondMatrix->cols; outputMatrix->isInit = 1;
    }
...
__host__ __device__ void multiplyScalarMatrix1D(float scalar, Matrix1DT *matrix, Matrix1DT *outputMatrix)
{
    ...
    outputMatrix->rows = matrix->rows; outputMatrix->cols = matrix->cols; outputMatrix->isInit = 1;
...
__host__ __device__ void LU_getInverseMatrix1D(Matrix1DT *inputMat, Matrix1DT *outInvMat)
{
    ...
    if (0 == outInvMat->isInit)
    {
        ...
        outInvMat->rows = inputMat->rows; outInvMat->cols = inputMat->cols; outInvMat->isInit = 1;
    }
...
__host__ __device__ void allocateMatrix1D(Matrix1DT* matrix, int rows, int cols)
{
    ...
    error = cudaMalloc((void**)&matrix->pMatrix[0], matrix->rows * matrix->cols * sizeof(float));
